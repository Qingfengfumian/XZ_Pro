...
def data_loader(train_date, distr,ftype):
    ...
    Train_List = []
    try:
        train_data = pd.read_csv('./Data/{}/Alert_Samp/Samp_train_XJ_{}/故障_样本_{}_5G.csv'.format(distr,ftype, train_date),
                                     encoding='gbk',engine='python',index_col=0)  # index_col=0)
    except:
        train_data = pd.read_csv(r'./Data/{}/Alert_Samp/Samp_train_XJ_{}/故障_样本_{}_5G.csv'.format(distr,ftype,train_date),
    ...
    mkdir(r'./Data/{}/Inter_Result/5G/{}_故障_标准化/'.format(distr,ftype))
    with open(r'./Data/{}/Inter_Result/5G/{}_故障_标准化/标准化_{}.pk'.format(distr,ftype,train_date), 'wb') as f:
        pickle.dump(ss, f)
        f.close()
    return x_train
def ae_model(x_train,ftype,distr,date):
    ...
    history = Ate.fit(x_train, x_train,
                       ...
                       shuffle=True)
    mkdir(r"./Data/{}/Inter_Result/5G/{}_故障_模型/".format(distr,ftype))
    Ate.save(r"./Data/{}/Inter_Result/5G/{}_故障_模型/模型AE_{}.h5".format
        (distr,ftype,date))
def vae_model(x_train,ftype,distr,date):
    ...
    # --------SaveModel--------
    mkdir(r"./Data/{}/Inter_Result/5G/{}_故障_模型/".format(distr,ftype))
    vae.save(r"./Data/{}/Inter_Result/5G/{}_故障_模型/模型VAE_{}.h5".format(distr,ftype,date))

def train_XJ_5G(para):
