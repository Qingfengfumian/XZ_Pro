...
def dict_gen(para,distr,mode,ftype):
    ...
    AlertHW_DF2 = pd.read_csv(
                        r'./Data/{}/Alert_Deal/Samp_{}_{}/故障_处理_{}_5G.csv'.format(distr, para.mode,ftype, date),encoding='gbk',
                        engine='python')
    ...
    if mode == 'train_XJ':
        ...
        mkdir(r'./Data/{}/Inter_Result/5G/{}_告警_标题/'.format(distr, ftype))
        with open(r'./Data/{}/Inter_Result/5G/{}_告警_标题/标题_{}.txt'.format(distr, ftype, date), 'wb') as f:
            pickle.dump(Alert_Title, f)
            f.close()
        print('完成-保存{}_告警标题'.format(ftype))
    else:
        # 读取告警标题
        title_filepath = find_new_file('./Data/{}/Inter_Result/5G/{}_告警_标题/'.format(distr, ftype))
    ...
    if mode == 'train_XJ':
        Sample_HW_df.to_csv(
        r'./Data/{}/Alert_Samp/Samp_{}_{}/故障_样本_{}_5G.csv'.format(distr, mode,ftype, date), index=False, encoding='gbk')
        print('巡检训练：{}_{}_{}样本生成 in {} seconds'.format(distr, ftype, date, (time.clock() - start)))
    if mode == 'predict':
        Sample_HW_df.to_csv(
        r'./Data/{}/Alert_Samp/Samp_{}_{}/故障_样本_{}_5G.csv'.format(distr,mode, ftype, para.date), index=False, encoding='gbk')
