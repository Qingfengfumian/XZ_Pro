...
from utils import real_warns_4G,load_model_infos,find_new_file2,save_model_infos,mkdir
...
def load_data(para,mode,distr,day=7):
    ...
    data_tmp = pd.read_csv(
        './Data/{}/Alert_Deal/Samp_{}_{}/故障_处理_{}_delJZ_4G.csv'.format(distr, mode,ftype, date),
    ...
    def trandform_grp_data(grp_sd):
        ...
        grp_sd2 = grp_sd.loc[
            ((grp_sd["data_hour"] <= 6) | (grp_sd["data_hour"] >= 23)) & (
                grp_sd["告警名称"].isin(real_warns_4G))]  # 小时数不完整，空的自动不计入  # 23-6
        #         grp_sd2 = grp_sd.loc[
        #             (grp_sd["告警名称"].isin(real_warns_4G))]  # 全天
        del grp_sd
        return grp_sd2
...
def gen_model_data(para,day,distr,source_data, warn_scores, input_len=7, target_long=3):
    ...
    if para.mode == "predict":
        fea_title_path = "./Data/{}/Inter_Result/4G/{}_特征标题/".format(distr, para.ftype)
    ...
    if para.mode == 'train_TF':
        ft_file = "./Data/{}/Inter_Result/4G/{}_特征标题/input_features_del{}_{}.txt".format(distr,para.ftype,day,
                                                                                para.train_date1)
        ...
        Xdata.to_csv(r"./Data/{}/Alert_Samp/Samp_{}/退服_样本_Xdata_del{}_{}_4G.csv".format(distr, para.ftype, day, para.train_date1),
                     index=False)
        ydata.to_csv(r"./Data/{}/Alert_Samp/Samp_{}/退服_样本_ydata_del{}_{}_4G.csv".format(distr, para.ftype, day, para.train_date1),
                     index=False)
...
def proc_TF_4G(para,distr,day):
    ...
    if (mode=='predict') | (mode == "evaluate"):
        fea_cod_path = "./Data/{}/Inter_Result/4G/{}_特征编码/".format(distr,para.ftype)
