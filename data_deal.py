...
def split_distr(alert_data,file_date,distr_list):
    ...
    alert_data.columns=['设备类型','告警标题','告警发生时间','告警级别','定位信息','设备厂家名称','网元名称','县市','地区','告警工程状态','专业','基站编号','清除时间','设备机房']
    alarm_sele = alert_data[(alert_data['设备类型'] == 'ENodeB') | (alert_data['设备类型'] == 'GNodeB')]
    alert_data = alarm_sele[['网元名称', '设备类型', '告警发生时间', '设备厂家名称', '清除时间', '告警标题']]
    nm_date = pd.read_csv('./Data/汇总/Project_Opt/GC-CELL-BS-ROOM.csv', encoding='gbk')
    nm_ID = nm_date[['基站名称', 'ENODEB_ID']]
    merge_data1 = pd.merge(alert_data, nm_ID, how='left', left_on='网元名称', right_on='基站名称')
    merge_data1.drop(columns=['基站名称'], inplace=True)
    nm_AREA = nm_date[['基站名称', '所属地市']]
    merge_data2 = pd.merge(merge_data1, nm_AREA, how='left', left_on='网元名称', right_on='基站名称')
    merge_data2.drop(columns=['基站名称'], inplace=True)
    alarm_sele = merge_data2.dropna(subset=['ENODEB_ID','所属地市'])
    # print(merge_data2.shape,alarm_sele.shape)
    alert_data = alarm_sele.copy()

    alert_data.rename(
        columns={'网元名称': '基站名称', '所属地市': '地市名称', '设备类型': '网络类型',
                 '清除时间': '告警清除时间', '告警标题': '告警标题'}, inplace=True)
    alert_4G = alert_data[(alert_data['网络类型'] == 'ENodeB') | (alert_data['网络类型'] == 'GNodeB')]
    alert_4G['网络类型'] = alert_4G['网络类型'].map(lambda x: 103 if x == 'ENodeB' else 108)
    # alert_4G['地市名称'] = '全省'
    ...
    try:
        alert_dropna['告警发生时间'] = alert_dropna['告警发生时间'].map(lambda x: datetime.strptime(x, "%Y-%m-%d %H:%M:%S"))
    except:
        alert_dropna['告警发生时间'] = alert_dropna['告警发生时间'].map(lambda x: datetime.strptime(x, "%Y/%m/%d %H:%M"))
    ...
    Alert_XJ['区域（地市）'] = Alert_XJ['区域（地市）'].map(lambda x: x.split('盟')[0])
    ...
    for tr_date in date_list:
        ...
        for i_region in distr_list:
            ...
            if (os.path.exists(alarm_path) == False) & (run_split_first == True):
                try:
                    alert_data = pd.read_csv("{}/wuxian_alarm_{}.csv".format(para.alarm_path, tr_date))
                except:
                    try:
                        alert_data = pd.read_csv("{}/wuxian_alarm_{}.csv".format(para.alarm_path, tr_date),encoding='gbk',sep='\$\$',header=None)
                    except:
                        print("Error: 没有找到文件或读取文件失败:{}".format("{}/wuxian_alarm_{}.csv".format(para.alarm_path, tr_date)))
                        os._exit(0)
...
def concat_date(para):
    ...
    for ftype in ftype_lis:
        for distr in distr_list:
            IF_exe_code = 0
            if ftype == '华为':
                if distr in ['拉萨','昌都','山南','林芝']:
                    IF_exe_code = 1
            elif ftype == '中兴':
                if distr in ['日喀则','那曲','阿里']:
                    IF_exe_code = 1
            if IF_exe_code == 1:
                tempall_4G = []
                temp2all_4G = []
                ...
                if len(tempall_4G) !=0:
                    ...
                    print('{}{} tempall_4G is null!'.format(distr,ftype))
                if len(temp2all_4G) != 0:
                    AlertHW_DF22 = pd.concat(temp2all_4G, axis=0)
                    AlertHW_DF22['告警名称'] = AlertHW_DF22['告警名称'].map(lambda x: x.replace('\/', '/'))
                    AlertHW_DF22.to_csv('./Data/{}/Alert_Deal/Samp_{}_{}/故障_处理_{}_delJZ_4G.csv'.format(distr,mode, ftype, date),index=False,encoding='gbk')
                else:
                    print('{}{} temp2all_4G is null!'.format(distr,ftype))
                ...
                if len(tempall_5G) !=0:
                    ...
                    print('{}{} tempall_5G is null!'.format(distr, ftype))
                if len(temp2all_5G) !=0:
                    AlertHW_DF33 = pd.concat(temp2all_5G, axis=0)
                    AlertHW_DF33['告警名称'] = AlertHW_DF33['告警名称'].map(lambda x: x.replace('\/', '/'))
                    AlertHW_DF33.to_csv('./Data/{}/Alert_Deal/Samp_{}_{}/故障_处理_{}_delJZ_5G.csv'.format(distr, mode, ftype, date),index=False, encoding='gbk')
                else:
                    print('{}{} temp2all_5G is null!'.format(distr, ftype))
...
#         alert_data_del1 = pd.read_csv("./Data/{}/Alert_Data/故障_{}_delJZ_4G.csv".format(i_region, tr_date),encoding='gbk')
#     except:
#         print("Error: 没有找到文件或读取文件失败:{}".format("./Data/{}/Alert_Data/故障_{}_4G.csv".format(i_region, tr_date)))
#         return (0)
#         # os._exit(0)
#     for i_ftype in ftype_list:
#         IF_exe_code = 0
#         if i_ftype == '华为':
#             if i_region in ['沈阳','大连','鞍山','本溪','丹东','营口','阜新','辽阳','铁岭','朝阳']:
#                 IF_exe_code = 1
#         elif i_ftype == '中兴':
#             if i_region in ['沈阳','大连','鞍山','抚顺','本溪','锦州','朝阳','营口','盘锦','葫芦岛']:
#                 IF_exe_code = 1
#         if IF_exe_code == 1:
#             alert_data = alert_data1[alert_data1['厂家'] == i_ftype]
#             alert_data_del = alert_data_del1[alert_data_del1['厂家'] == i_ftype]
