...
def data_loader(distr, date, ftype):
    ...
    Test_samp = pd.read_csv(r'./Data/{}/Alert_Samp/Samp_predict_{}/故障_样本_{}_5G.csv'
                                 .format(distr,ftype, date), encoding='gbk', index_col=0,engine='python') 
    x_test = Test_samp.values[:, :]
    # 标准化
    scaled = np.copy(x_test)
    ss_filepath = find_new_file( "./Data/{}/Inter_Result/5G/{}_故障_标准化/".format(distr,ftype))
...
def ae_predictor(distr, x_pred, ftype):
    ...
    model_filepath = find_new_model_file(
        r"./Data/{}/Inter_Result/5G/{}_故障_模型/".format(distr, ftype), modtype='模型AE')
    model = load_model(r"./Data/{}/Inter_Result/5G/{}_故障_模型/{}".format(distr, ftype, model_filepath),custom_objects={'mean_squa': mean_squa})
...
def vae_predictor(distr, x_pred, ftype):
    ...
    start = time.clock()
    model_filepath = find_new_model_file(r"./Data/{}/Inter_Result/5G/{}_故障_模型/".format(distr,ftype), modtype='模型VAE')
    vae = load_model(r"./Data/{}/Inter_Result/5G/{}_故障_模型/{}".format(distr,ftype, model_filepath),
...
def predict_XJ_5G(para):
    ...
    for distr in distr_list:
        ...
        Fin_result.to_csv(
            "./Data/{}/Inspect_List/{}/Origin_{}_5G.csv".format(distr, ftype, date),
            index=None,encoding='gbk')
        print('保存-巡检清单 in <./Data/{}/Inspect_List/{}/Origin_{}_5G.csv'.format(distr, ftype, date))
